...
static void synaptics_change_dvfs_lock(struct work_struct *work)
{
	...
	case TSP_BOOSTER_LEVEL9:
		...
		break;
	default:
		dev_dbg(&rmi4_data->i2c_client->dev, "%s: [TSP_DVFS] Undefined type passed %d\n",
			__func__, rmi4_data->boost_level);
		break;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		dev_dbg(dev, "%s: Failed to write proximity enables, error = %d\n",
				__func__, retval);
		return retval;
	}
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval != PAGE_SELECT_LEN) {
				if ((rmi4_data->tsp_probe != true) && (retry >= 1)) {
					dev_dbg(&i2c->dev, "%s: TSP needs to reboot \n", __func__);
					retval = TSP_NEEDTO_REBOOT;
					return retval;
				}
				dev_dbg(&i2c->dev, "%s: I2C retry = %d, i2c_master_send retval = %d\n",
		...
		if (finger_status) {
			...
			if (!rmi4_data->finger[finger].state) {
				...
				if (tsp_pattern_tracking(rmi4_data, finger, x, y, false)) {
					dev_dbg(&rmi4_data->i2c_client->dev, "Sunflower-Hopping (Pattern Tracking)\n");
				...
				if ((rmi4_data->finger[finger].mcount % TSP_PT_MOVE_COUNT_TH) == 0) {
					if (tsp_pattern_tracking(rmi4_data, finger, x, y, true)) {
						dev_dbg(&rmi4_data->i2c_client->dev, "Sunflower-Fixed (Pattern Tracking)\n");
						cancel_delayed_work(&rmi4_data->reboot_work);
						schedule_delayed_work(&rmi4_data->reboot_work,
							msecs_to_jiffies(TSP_PT_REBOOT_PENDING_TIME * 6)); /* 300msec*/
						return 0;
			...
			if (rmi4_data->finger[finger].state) {
				dev_dbg(&rmi4_data->i2c_client->dev, "[%d][R] 0x%02x M[%d], Ver[%02X%02X%02X %02X%02X]\n",
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (data->edge_swipe_dg >= 90 && data->edge_swipe_dg <= 180)
		...
		dev_dbg(&rmi4_data->i2c_client->dev, "Skip wrong edge swipe angle [%d]\n",
			data->edge_swipe_dg);
...
int synaptics_rmi4_glove_mode_enables(struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_dbg(&rmi4_data->i2c_client->dev, "%s: [%02X]: %s\n",
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	dev_dbg(&rmi4_data->i2c_client->dev, "%s: [IC] [date, revision, version, panel] [%02d/%02d, 0x%02X, 0x%02X, 0x%02X]\n",
...
int synaptics_proximity_no_sleep_set(bool enables)
{
	...
	if (retval <= 0)
		dev_dbg(&f51->rmi4_data->i2c_client->dev, "%s: fail to read no_sleep[ret:%d]\n",
				__func__, retval);
	...
	retval = synaptics_rmi4_i2c_write(f51->rmi4_data,
			...
			sizeof(no_sleep));
	if (retval <= 0)
		dev_dbg(&f51->rmi4_data->i2c_client->dev, "%s: fail to set no_sleep[%X][ret:%d]\n",
...
static int synaptics_rmi4_f51_set_enables(struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_dbg(&rmi4_data->i2c_client->dev, "%s: %s[0x%x]\n", __func__,
		...
		unsigned int intr_count)
{
	...
	if (retval < 0) {
		dev_dbg(&rmi4_data->i2c_client->dev, "%s: fail proximity set init\n",
			__func__);
		return retval;
	}
...
flash_prog_mode:
	...
	if (!list_empty(&rmi->support_fn_list)) {
		list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
			...
			dev_dbg(&rmi4_data->i2c_client->dev, "%s: F%02x : NUM_SOURCE[%02X] NUM_INT_REG[%02X] INT_MASK[%02X]\n",
				__func__, fhandler->fn_number,
...
static int synaptics_rmi4_open_lcd_ldi(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (IS_ERR(window_type)) {
		...
		if (iRet != -ENOENT)
			dev_dbg(&rmi4_data->i2c_client->dev, "%s: window_type file open fail\n", __func__);
		set_fs(old_fs);
		goto exit;
	}
	...
	if (iRet != (sizeof(u8) * 4)) {
		dev_dbg(&rmi4_data->i2c_client->dev, "%s: Can't read the lcd ldi data\n", __func__);
		iRet = -EIO;
	}
...
static void synaptics_rmi4_reboot_work(struct work_struct *work)
{
	...
	dev_dbg(&rmi4_data->i2c_client->dev, "%s: Tsp Reboot(%d) by pattern tracking\n",
	...
	retval = synaptics_rmi4_reset_device(rmi4_data);
	if (retval < 0) {
		dev_dbg(&rmi4_data->i2c_client->dev, "%s: Failed to issue reset command, error = %d\n",
				__func__, retval);
	}
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (!platform_data->get_ddi_type) {
		dev_dbg(&client->dev, "%s: Do not support getting DDI Type\n", __func__);
	} else {
		rmi4_data->ddi_type = platform_data->get_ddi_type();
		dev_dbg(&client->dev, "%s: DDI Type is %s[%d]\n",
...
err_tsp_reboot:
	...
	if (retval < 0) {
		dev_dbg(&client->dev, "%s: Failed to set up input device\n",
				__func__);
		...
		if ((retval == TSP_NEEDTO_REBOOT)
			...
			dev_dbg(&client->dev, "%s: reboot sequence by i2c fail\n",
					__func__);
			goto err_tsp_reboot;
		} else {
	...
	if (retval < 0) {
		dev_dbg(&client->dev, "%s: Failed to register rmidev module\n",
				__func__);
	...
	if (retval < 0) {
		dev_dbg(&client->dev, "%s: Failed to enable attention interrupt\n",
				__func__);
		goto err_enable_irq;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			dev_dbg(&client->dev, "%s: Failed to create sysfs attributes\n",
					__func__);
			goto err_sysfs;
		}
	}
